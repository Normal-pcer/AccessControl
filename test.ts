...
// 样本内容生成函数
function sampleContent(maxLength: number = 800): string {
...
const createBlocker = (): void => {
  ...
  `;
    ...
    const message = safeCreateElement(
        "div",
        {
    ...
    const footer = safeCreateElement(
        "div",
        {
...
// 移除拦截器
const removeBlocker = (): void => {
    if (blockerRemoved) return;
    blockerRemoved = true;
...
const showPermanentBlock = (): void => {
    ...
    const backButton = safeCreateElement(
        "button",
        {
            id: "ai-back-btn",
    ...
    const reportButton = safeCreateElement(
        "button",
        {
            id: "ai-report-btn",
    ...
    const finalFooter = safeCreateElement(
        "div",
        {
            style: "margin-top: 30px; font-size: 0.9rem; opacity: 0.7;",
        },
