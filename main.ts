...
namespace ContentGuard {
    ...
    export namespace Config {
        ...
        export interface AppConfig {
            ...
            saveMode: boolean; // 严格模式
            whitelist: string[]; // 白名单
            strictCount: number; // 接下来若干个网页处于严格模式
        }
        ...
        export interface BudgetData {
            ...
            dailyUsage?: {
                today: { date: string; tokens: number }; // 今日使用量
        ...
        export interface CreditData {
            amount: number; // 信用分
            safeTimestamps: number[]; // 最近三次安全标记的时间戳
        }
        ...
        export namespace CreditSystem {
            const DEFAULT_CREDIT = 0;
            const MIN_CREDIT = -10;
            const MAX_CREDIT = 10;

            export async function getCreditData(host: string): Promise<CreditData> {
                try {
                    const credits = await GM.getValue<Record<string, CreditData>>(CREDITS_KEY, {});
                    if (!credits[host]) {
            ...
            async function updateCreditData(host: string, data: CreditData): Promise<void> {
                try {
                    const credits = await GM.getValue<Record<string, CreditData>>(CREDITS_KEY, {});
                    credits[host] = data;
            ...
            export async function increaseCredit(host: string, amount: number): Promise<void> {
             ...
             * 更新最近三次安全标记的时间戳
             */
            export async function markSafe(host: string): Promise<void> {
                const creditData = await getCreditData(host);
                const now = Date.now();
             ...
             */
            async function isFrequentSafe(host: string): Promise<boolean> {
                const creditData = await getCreditData(host);
                const now = Date.now();
                ...
                // 检查所有时间戳是否都在1小时内
                return creditData.safeTimestamps.every(
             ...
             */
            export async function getProb(host: string): Promise<number> {
                // 检查安全标记条件
        ...
        export async function updateBudget(tokensUsed: number): Promise<void> {
            try {
                ...
                if (monthData.dailyUsage.today.tokens > dailyLimit) {
                    const message = `今日用量已达月预算 5%: ${monthData.dailyUsage.today.tokens.toLocaleString()}/${Math.floor(
                        dailyLimit
                    ).toLocaleString()} 字符`;
        ...
        export function isWhitelisted(url: string, patterns: string[]): boolean {
         ...
         * @returns 带通配符的域名格式
         */
        function convertToWildcardDomain(url: string): string {
            // 移除协议部分（如果存在）
         ...
         * 如果是，将会消耗一次计数
         */
        export async function isStrictMode(): Promise<boolean> {
            const config = await getConfig();
...
<div class="content-guard-config-section">
    ...
    <div class="content-guard-form-group">
        <label class="content-guard-form-label">
            <input type="checkbox" id="content-guard-config-strict-mode" ${
                config.saveMode ? "checked" : ""
            }>
            启用省token模式
        </label>
...
<div class="content-guard-btn-group">
    <button id="content-guard-save-whitelist" class="content-guard-btn content-guard-btn-primary">保存白名单</button>
            ...
            `;
                ...
                container?.addEventListener("click", (e) => {
                    if ((e.target as Element).classList.contains("content-guard-btn-delete")) {
                        (e.target as Element).parentElement?.remove();
                    }
                });
                    ...
                    ?.addEventListener("click", async () => {
                        const inputs = Array.from(
                            document.querySelectorAll(
                                "#content-guard-whitelist-container .content-guard-whitelist-item input"
                            )
                        ) as HTMLInputElement[];
                ...
                async function saveApiConfig() {
                    const apiKeyInput = document.getElementById(
                        "content-guard-config-api-key"
                    ) as HTMLInputElement;
                    const baseUrlInput = document.getElementById(
                        "content-guard-config-base-url"
                    ) as HTMLInputElement;
                ...
                async function saveBehaviorConfig() {
                    const strictModeInput = document.getElementById(
                        "content-guard-config-strict-mode"
                    ) as HTMLInputElement;
                ...
                async function resetBudget() {
                    try {
                        const now = new Date();
                        const monthKey = `${now.getFullYear()}-${now.getMonth() + 1}`;
                        const budget = await GM.getValue<Record<string, BudgetData>>(
                            BUDGET_KEY,
                            {}
                        );
                ...
                function showStatus(message: string, isSuccess: boolean) {
                    ...
                    statusDiv.className = `content-guard-status-message ${
                        isSuccess ? "content-guard-status-success" : "content-guard-status-error"
                    }`;
    ...
    export namespace Blocker {
        ...
        export class NoneBlocker implements IBlocker {
            execute(): void {
                // 不进行操作
            }
        }
         ...
         * 将接下来几次访问标记为严格模式
         */
        export class MarkStrictBlocker implements IBlocker {
        ...
        export class CooldownBlocker implements IBlocker {
            private static readonly OVERLAY_ID = "content-guard-cooldown-challenge-overlay";
            private static readonly TIMER_ID = "content-guard-cooldown-timer";
            private static readonly CHALLENGE_COUNTER_ID = "content-guard-challenge-counter";
            private static readonly QUESTION_CONTAINER_ID = "content-guard-question-container";
            private static readonly ANSWER_INPUT_ID = "content-guard-answer-input";
            private static readonly SUBMIT_BUTTON_ID = "content-guard-submit-button";
            private static readonly STATUS_ID = "content-guard-challenge-status";
         ...
         * 将会依次执行几个拦截器
         */
        export class CombinedBlocker {
            constructor(private blockers: IBlocker[]) {}
            execute(): void {
    ...
    export namespace PreBlocker {
        ...
        export const remove = (lock: Lock): void => {
            if (!blockerCreated || !blockerElement) return;
    ...
    export namespace Core {
        ...
        function getCreditByRating(rating: Rating): number {
...
搜索关键词：${query}`;
         ...
         * 2. 处于测试模式，存在相关选项
         */
        export async function ensureSkip(config: Config.AppConfig | null = null): Promise<boolean> {
            if (config === null) {
        ...
        export async function checkPage(): Promise<void> {
            ...
            if (!searchMode) {
                ...
                blockByRating(rating);
                await Config.CreditSystem.increaseCredit(
                    window.location.hostname,
                    getCreditByRating(rating)
                );
...
(function () {
    ...
    const hasContent = () => {
        ...
        if (visibleText.length < 100) return false;
     ...
     */
    const conditionalCheck = async (force: boolean = false) => {
        if (ensureSkip) return;
        if (checked) return; // 始终不重复检查
        if (force || hasContent()) {
            checked = true;
